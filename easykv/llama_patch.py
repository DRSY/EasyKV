...
def apply_rotary_pos_emb_sep(x, cos, sin, position_ids, unsqueeze_dim=1):
    """Applies Rotary Position Embedding to the query and key tensors.
...
def llama_forward(
    ...
    **kwargs,
    ...
    # for KV eviction purpose
    max_Pos = torch.topk(position_ids[0], k=1, dim=-1)[0].cpu().item()+1
    cos, sin = self.rotary_emb(
        value_states, seq_len=max(kv_seq_len, max_Pos))
...
def llama_forward_stream(
    ...
    **kwargs,
    ...
    if past_key_value is not None:
        ...
        prev_kv_length = past_key_value.get_usable_length(kv_seq_len, self.layer_idx)
        query_position_ids = torch.arange(prev_kv_length, prev_kv_length+query_states.shape[-2], device=attn_device).unsqueeze(0).repeat(bsz, 1)
        key_position_ids = torch.arange(0, prev_kv_length+query_states.shape[-2], device=attn_device).unsqueeze(0).repeat(bsz, 1)
        kv_seq_len += past_key_value.get_usable_length(
            kv_seq_len, self.layer_idx)
    else:
        query_position_ids = torch.arange(0, query_states.shape[-2], device=attn_device).unsqueeze(0).repeat(bsz, 1)
        key_position_ids = torch.arange(0, query_states.shape[-2], device=attn_device).unsqueeze(0).repeat(bsz, 1)
    cos, sin = self.rotary_emb(
    ...
    query_states = apply_rotary_pos_emb_sep(query_states, cos, sin, query_position_ids)
    key_states = apply_rotary_pos_emb_sep(key_states, cos, sin, key_position_ids)
